       ...
       if msg.reply_to_message and msg.reply_to_message.sticker:
            ...
            try:
               ...
               bot.sendDocument(chat_id, document=open('sticker.png', 'rb'))
                   ...
                   update.effective_message.reply_text("Dammit, got some errors while processing the sticker, you may report it to my master - (@stellatm) if the error persists.")
...
def makepack_internal(msg, user, png_sticker, emoji, bot):
    ...
    packname = f"c{user.id}_by_{bot.username}"
    try:
        success = bot.create_new_sticker_set(user.id, packname, name + "'s stella pack",
